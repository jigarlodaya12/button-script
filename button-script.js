...
var IM = (function() {
  ...
  var opt1Values = [],
    ...
    opt3Values = [];
  var BABModal =
    '<div class="bab-modal-overlay" id="bab_cart_overlay" onclick="IM.hideCart()"></div>' +
  ...
  var BABProductContainer =
    '<div class="bab-product-item-img">' +
  ...
  var partials = {
    quantity:
      '<div class="bab-quantity-ctrl" id="bab_quantity_ctrl">' +
      '          <label for="">Qty</label>' +
  ...
  var pincodeWidget =
    '<form class="bab-pincode-form" novalidate>' +
  ...
  var product_options_widget =
    '<form class="bab-pincode-form" id="product_options_main" onsubmit="IM.checkProductOptionsValidation()">' +
    '    <div class="bab-select-wrapper">' +
    "    {{#productOptions}} " +
    "      {{#text}}" +
  ...
  "  </form>" + '<div id="shipping_option_detail"></div>';

  var checkoutForm =
    '<form id="bab-checkout-form" action="{{user_domain}}/checkout/" method="POST" enctype="application/x-www-form-urlencoded;charset=utf-8">' +
  ...
  var optionsTemplate =
    '<select class="bab-select-box" id="{{key_id}}" name="variant-select-box" onChange="IM.renderOptionsTemplate({{key_id}})">' +
    "  {{#optionValues}}" +
    "      {{#selected}}" +
  ...
  var shippingOptionTemplate =
    '<ul class="bab-shipping-detail-list">' +
    "  {{#cod}}" +
  ...
  function fetchProductDetails(params) {
    ...
    request({
      ...
      var iframeHtml = Mustache.to_html('<div id="iframe-bg"><iframe id="myIframe" height="100%" width="100%"></iframe><span onclick="IM.hideIframe()" class="close"></span></div>');
      ...
      if (data.objects.length) {
        ...
        if (data.objects.length > 1) {
          ...
          availableVariants = data.objects.reduce(function(prev, curr) {
            var combo = [curr.optv1, curr.optv2, curr.optv3]
              .filter(function(val) {
                return val;
              })
              .join("|");
    ...
    for (var i = 0; i < activeProduct.options.length; i++) {
      if (activeProduct.options[i].option_id == option_id) {
        ...
        if (type == "checkbox") {
          ...
          if (value || value != "") activeProduct.options[i].value = value;
          else activeProduct.options.splice(i, 1);
        }
      }
    }
  ...
  function convertLinkToButton(randomId, checkoutButtonData, element) {
    ...
    let changeString =
      `<div class=${"bab_product_container_" + checkoutButtonData.product_id}><div class="im-checkout btn-${randomId} bab-product-item-img ">
      <a href="javascript:void(0)" class="bab-button im-checkout-btn btn--default" onClick=IM.showCart(` +
